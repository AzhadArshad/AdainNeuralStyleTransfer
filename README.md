...
> **Trained from scratch.** No pretrained style transfer model was used — the decoder network was designed and trained entirely by us, using our own training pipeline (`train.py`). Only the VGG19 encoder weights (used purely as a frozen feature extractor) are borrowed from a standard pretrained backbone.
...
*Built with PyTorch + Flask · Deployed on Render*
